...
Maybe a quick answer to an important question: the testbed does not maintain time-frequency-phase-amplitude synchronization with the cable, GPS, or over the network switch. It is purely based on PPDU (written in Python - can be modified) exchanges between the SDRs and a triggering mechanism allowing flexible IQ data transmission and reception.
